...
// `tools` and `systemPrompt` are provided by the base bundle as sibling rows of the
// same composed tree (dsh-base's own patch is one flat `insert`, with no `group`
// nesting), so the services are visible to every community bundle. Inject them
// statically like the working third-party bundles do (@etony668/dsh-task-board
// injects `tools`/`skills` this way). Static injection also fails loud instead of
// leaving the decision tools silently unregistered.
