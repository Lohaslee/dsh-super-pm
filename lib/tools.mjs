...
export function apply(ctx) {
  ...
  c.tools.register(defineTool({
    ...
    parameters: { phrase: string("User-provided phrase or context."), completed_decisions: { type: "number", description: "Optional number of completed product decisions." }, random: { type: "boolean", description: "Allow a low-probability stateless random discovery." } },
    output: { schema: { type: "object", additionalProperties: true }, render: (_args, value) => text(value) },
    isConcurrencySafe: () => true,
    async execute(args) {
      return easterEgg(args.phrase, { completedDecisions: args.completed_decisions, random: args.random === true });
    }
  }));
